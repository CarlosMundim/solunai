...
const ScenarioAssessment: React.FC<ScenarioAssessmentProps> = ({ lang = 'en' }) => {
  ...
  const generatePDF = () => {
    ...
    doc.text('SCE PROFILE TYPE（プロファイルタイプ）', 20, yPos);

    yPos += 10;
    doc.setFontSize(7);
    doc.setTextColor(100, 100, 100);
    doc.text('Solunai Continuity Engine（SCE）がJFCスコアから行動傾向を要約したプロファイルです。', 20, yPos);
    ...
    // SCE Management Guidance Section
    yPos += 10;
    doc.setFillColor(255, 247, 237);
    doc.rect(15, yPos - 5, pageWidth - 30, 8, 'F');
    doc.setFontSize(11);
    doc.setTextColor(234, 88, 12);
    doc.text('SCE MANAGEMENT GUIDANCE（マネジメントガイダンス）', 20, yPos);

    yPos += 10;
    doc.setFontSize(7);
    doc.setTextColor(100, 100, 100);
    doc.text('JFCプロファイルに基づき、マネジメントやオンボーディングで意識すべきポイントをSCEが提案します。', 20, yPos);

    yPos += 6;
    doc.setFontSize(8);
    doc.setTextColor(60, 60, 60);

    result.SCE_MANAGEMENT_GUIDANCE.forEach(guidance => {
      const splitGuidance = doc.splitTextToSize(`• ${guidance}`, 170);
      doc.text(splitGuidance, 20, yPos);
      yPos += splitGuidance.length * 4 + 2;
    });
    ...
    doc.setTextColor(100, 100, 100);
    doc.setFont('helvetica', 'italic');
    doc.text('Generated by Solunai Continuity Engine (SCE) based on JFC profile.', 20, yPos);
    yPos += 3;
    doc.text('（JFCプロファイルに基づきSolunai Continuity Engineが自動生成）', 20, yPos);
    doc.setFont('helvetica', 'normal');

    // SCE Integration Timeline Section with JP Localization
    yPos += 10;
    doc.setFillColor(254, 249, 195);
    doc.rect(15, yPos - 5, pageWidth - 30, 8, 'F');
    doc.setFontSize(11);
    doc.setTextColor(161, 98, 7);
    doc.text('SCE INTEGRATION TIMELINE（チーム統合までの推定期間）', 20, yPos);

    yPos += 10;
    doc.setFontSize(7);
    doc.setTextColor(100, 100, 100);
    doc.text('JFCスコアに基づき、チームや企業文化への適応に必要な期間をSCEが推定します。', 20, yPos);

    yPos += 6;
    const timeline = result.SCE_INTEGRATION_TIMELINE;
    ...
    doc.setTextColor(100, 100, 100);
    doc.setFont('helvetica', 'italic');
    doc.text('Generated by Solunai Continuity Engine (SCE) based on overall JFC profile.', 20, yPos);
    yPos += 3;
    doc.text('（JFCプロファイル全体に基づきSolunai Continuity Engineが自動生成）', 20, yPos);
    doc.setFont('helvetica', 'normal');
